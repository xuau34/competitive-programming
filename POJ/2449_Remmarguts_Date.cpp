/*
  ...
  A*的走訪同Dijkstra。
  經測試，單純用S -> x的距離來去評估的話，會MLE。
  因此用A*是需要的。（目前是這樣，也沒看到其他人真用Dijkstra解決的。
 */
...
int sol(void){
  ...
  int x, y;
  priority_queue< pair<int, int>, vector<pair<int, int> >, greater< pair<int, int> > > que;
  que.push( make_pair(arr[S], S) );
  while(!que.empty()){
    //printf("(%d, %d) => ", que.top().first,  que.top().second);
    x = que.top().second; y = que.top().first - arr[x]; que.pop();
    if(x == T && !(--K)) return y;
    //printf("K = %d\n", K);
    for(int i = 0, j = no[ x ].size(); i < j; ++i)
      que.push( make_pair(y + no[x][i].second + arr[ no[x][i].first ], no[x][i].first) );
  }
  return -1;
