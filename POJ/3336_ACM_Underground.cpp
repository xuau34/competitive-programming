...
Line Turn(Point s, Point e){
  return Line(s.y - e.y, e.x - s.x, s.x * e.y - e.x * s. y);
}
...
void ini(){
  ...
  for(i = 0; i < n; ++i) scanf("%lf%lf%lf%lf", &seg[i].s.x, &seg[i].s.y, &seg[i].e.x, &seg[i].e.y);
  for(i = 0; i < m; ++i) scanf("%lf%lf", &Po[i].x, &Po[i].y), Po[i].flag = 1;
  sort(Po, Po + m);
  ...
  sort(set, set + tot);
  for(i = j = 0; i < tot; ++i){
    if(!bin(Po, m, set[i]) && (!i || !(set[i] == set[i - 1]))) set[j++] = set[i];
  }
  tot = j;
  for(i = 0; i < tot; ++i) set[i].flag = 0;
  for(i = 0; i < m; ++i){
    int cnt = 0;
    for(j = 0; j < n; ++j)
...
void build(){
  for(int i = 0; i <= n; ++i) dot_set_onseg[i].clear();
  for(int i = 0; i <= tot; ++i) edge[i].clear();
  for(int i = 0; i < n; ++i)
    for(int j = 0; j < tot; ++j)
      if(dotOnSeg(set[j], seg[i].s, seg[i].e)) dot_set_onseg[i].push_back(j);
  for(int i = 0; i < n; ++i){
    for(int j = 0, s = dot_set_onseg[i].size(); j < s; ++j){
      while(j < s - 1 && set[dot_set_onseg[i][j]].flag) ++j;
      if(j == s - 1) break;
      if(!set[dot_set_onseg[i][j + 1]].flag){
        int a = dot_set_onseg[i][j], b = dot_set_onseg[i][j + 1];
        edge[a].push_back(b);
        edge[b].push_back(a);
      }
    }
...
bool bfs(){
  ...
  int st = lower_bound(set, set + tot, from) - set, ed = lower_bound(set, set + tot, to) - set;
  queue<int> Q;
  Q.push(st), vis[st] = 1;
  while(!Q.empty()){
    int fr = Q.front();
    Q.pop();
    for(int i = 0, j = edge[fr].size(); i < j; ++i){
