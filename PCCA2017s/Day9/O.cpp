...
long long Pollard_rho(long long x, long long c){
  long long i = 1, k = 2, x0 = lrand() % x, y = x0;
...
void findfac(long long n){
  ...
  long long p = n;
  while(p >= n) p = Pollard_rho(p, lrand() % (n - 1) + 1);
...
int main(void){
  ...
  */
  ...
  while(N--){
    scanf("%lld", &i);
    if(Miller_Rabin(i) && Miller_Rabin(i)){
      printf("Prime\n");
      continue;
    }
    findfac(i);
